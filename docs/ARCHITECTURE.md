...
| | `plan_status` | reads |
| | `propose_plan_for_track` | reads — proposes, does not apply |
...
`propose_plan_for_track` proposing rather than applying is deliberate: a plan an agent can inspect before
